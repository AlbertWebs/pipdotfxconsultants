...
class AdminsController extends Controller
{
    ...
    public function homevideos(){
        activity()->log('Accessed Homepage Videos');
        $vids = Vid::all();
        $page_title = 'list';
        $page_name = 'Banners';
        return view('admin.vids',compact('page_title','vids','page_name'));
    }

    public function editHomeVideo($id){
        activity()->log('Accessed Edit Home Video ID number '.$id.' ');
        $Vids = Vid::find($id);
        $page_title = 'formfiletext';
        $page_name = 'Site Banner';
        return view('admin.editHomeVideo',compact('page_title','Vids','page_name'));
    }

    public function edit_HomeVideo(Request $request, $id){
        activity()->log('Editited Home Video ID number '.$id.' ');
        ...
        DB::table('vids')->where('id',$id)->update($updateDetails);
        Session::flash('message', "Changes have been saved");
        return Redirect::back();
    }
    ...
    public function comments(){
        ...
        $page_title = 'list';
        $page_name = 'Blog';
        return view('admin.comments',compact('page_title','Blog','page_name'));
    }
    public function approveComment($id){
        activity()->log('Accessed the approve comments section ');
        $Com = Comment::find($id);
        if($Com->status == 1){
            $newStatus = "0";
        }else{
    ...
    public function enroll_user_now(Request $request){
        ...
        if($course_id == 1){
            $course_id_var = "1";
            $updateDetails = array(
                'course_registered_id' => $course_id_var,
            );
        }else{
            $course_id_var = "2";
            $updateDetails = array(
                'course_2_registered_id' => $course_id_var,
            );
        }
